...
def batch_process_and_save(all_split, retriever, save_path, batch_size=64):
    output_data = []

    print('Start Retrieving ...')

    for i in tqdm(range(0, len(all_split), batch_size)):
        batch = all_split[i:i+batch_size]
        input_query = [sample['question'] for sample in batch]

        results, scores = retriever.batch_search(input_query, return_score=True)

        for j, sample in enumerate(batch):
            # print(sample)
            for item in results[j]:
                if isinstance(item, dict):
                    item['title'] = item['contents'].split("\n")[0].strip()
                    item['text'] = "\n".join(item['contents'].split("\n")[1:])
            output_data.append({
                'question': sample['question'],
                'answers': sample.get('answers') if sample.get('answers') is not None else sample.get('golden_answers').tolist(),
                'qa_pairs': sample.get('qa_pairs'),
                'ctxs': results[j]  # 每个 query 对应一组 ctxs
            })

    print(f'Finished retrieving for {len(output_data)} questions.')
...
def read_jsonl(file_path):
    with open(file_path, "r", encoding="utf-8") as f:
        data = json.load(f)  # 注意：这里是 json.load，不是 json.loads
    return data
...
def get_dataset(config):
    """Load dataset from config: support .json, .jsonl, and .parquet"""
    ...
    parquet_path = os.path.join(base_path, f'{split}.parquet')

    if os.path.exists(json_path):
        return read_jsonl(json_path)
        ...
        raise FileNotFoundError(f"No dataset found for split '{split}' in path '{base_path}'")
...
if __name__ == '__main__':
    ...
    save_path = os.path.join(args.save_dir, f'{args.data_split}.json')
    batch_process_and_save(all_split, retriever, save_path, batch_size=64)
